...
export default async function DistrictPage({
  ...
  const installTypeNote =
    config.housingType === 'mansion'
      ? `${config.name}周辺は高層マンション・集合住宅が多く、PS標準設置型・PS扉内設置型・PS扉内上方排気型などの設置タイプが中心です。扉の開口や排気の向きによって適合機種が限られるため、PS扉を開けた状態と本体・型番シールの写真でご確認ください。`
      : config.housingType === 'detached'
        ? `${config.name}周辺は戸建て住宅が多く、屋外壁掛型・据置型が設置タイプの中心です。ドレン排水の経路を確保しやすいため、省エネ型のエコジョーズへの更新もスムーズなケースが多くなっています。`
        : `${config.name}周辺はマンションと戸建てが混在し、マンションのPS設置型と戸建ての屋外壁掛型・据置型の両方の交換に対応しています。住宅タイプによって設置タイプが異なるため、設置状況の写真でご確認のうえ最適な機種をご提案します。`
    ...
    <>
      ...
      <main className="pt-[100px]">
        ...
        <section id="dt-area" className="py-10 bg-white scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-xl font-black text-gray-900 mb-4">{config.name}周辺の住宅事情</h2>
        ...
        {/* 設置タイプ */}
        <section id="dt-installtype" className="py-10 bg-gray-50 scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-xl font-black text-gray-900 mb-4">{config.name}周辺で多い設置タイプ</h2>
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              <p className="text-sm text-gray-700 leading-relaxed">{installTypeNote}</p>
            </div>
          </div>
        </section>

        {/* 号数の選び方 */}
        <section id="dt-capacity" className="py-10 bg-white scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
        ...
        {/* エコジョーズ */}
        <section id="dt-eco" className="py-10 bg-gray-50 scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-xl font-black text-gray-900 mb-4">
              {config.name}周辺でエコジョーズに交換する
            </h2>
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              <p className="text-sm text-gray-700 leading-relaxed">
                エコジョーズは排気熱を再利用する熱効率95%以上の省エネ型給湯器で、従来型と比べてガス代を約10〜15%削減できる場合があります。
                {config.housingType === 'detached'
                  ? `${config.name}周辺の戸建ては、運転時に発生する凝縮水（ドレン排水）の経路を確保しやすいため、エコジョーズへの交換に適したケースが多くなっています。お湯の使用量が多いご家庭ほど省エネ効果が大きくなります。`
                  : config.housingType === 'mansion'
                    ? `${config.name}周辺のマンションでエコジョーズへ交換する場合は、運転時に発生する凝縮水（ドレン排水）の処理について管理規約で取り扱いが定められていることがあるため、事前の確認が必要です。現在の機種がエコジョーズかどうか、ドレン排水口の有無もあわせて確認します。`
                    : `${config.name}周辺は戸建てとマンションが混在するため、住宅タイプによって設置条件が変わります。戸建てではドレン排水の経路を確保しやすく交換しやすい一方、マンションでは排水処理について管理規約の確認が必要な場合があります。`}
        ...
        {/* 工事費込み */}
        <section id="dt-cost" className="py-10 bg-gray-50 scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-xl font-black text-gray-900 mb-4">
              {config.name}周辺での給湯器交換費用について
            </h2>
            <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        {/* FAQ */}
        <section id="dt-faq" className="py-10 bg-gray-50 scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4">
        ...
        <section className="py-10 bg-gray-50">
          <div className="max-w-4xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <p className="text-xs font-bold text-gray-500 mb-2">最寄り駅・横浜市{config.wardName}</p>
